...
window.refreshCurrentPage = function () {
  ...
  switch (activePage) {
    ...
    case 'inv-check': renderInvCheckPage(); break;
...
function parseBomText(text, separator) {
  ...
  // ヘッダー解析
  // Excelからのコピペなどはタブ区切り、CSVアップロードはカンマ区切り
  // separatorが指定されていればそれを使う
  let headers = lines[0].split(separator).map(c => c.replace(/^"|"$/g, '').trim());

  let processNames = [];
  let startIndex = 0; // データ開始行

  // 標準的なヘッダーが含まれているか確認
  // (A:カテゴリ, B:製品名, C:BOM名, D:部材CD ... 以降は工程)
  // ヘッダー行っぽければ、そこから工程名を抽出する
  if (headers.includes('製品名') || headers.includes('BOM名') || headers.includes('部材CD')) {
    startIndex = 1; // 1行目はヘッダー
    // 4番目以降を工程名として取得 (0:カテゴリ, 1:製品名, 2:BOM名, 3:部材CD)
    if (headers.length > 4) {
      processNames = headers.slice(4).filter(h => h && h.length > 0);
    }
  } else {
    // ヘッダーなしの場合、工程は取り込めない
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    // 必須列チェック (BOM名, 部材CD)
    // index: 0=Cat, 1=Prod, 2=BomName, 3=PartCode
  ...
  const body = `
        <div class="form-group">
            ...
            <p class="text-muted" style="font-size:0.8rem;">
              ※1行目にヘッダー（カテゴリ, 製品名, BOM名, 部材CD, [工程1], [工程2]...）を含めると工程も取り込めます。
            </p>
            <textarea id="bom-paste-area" class="form-input" style="height: 200px; font-family: monospace;" placeholder="カテゴリ	製品名	BOM名	部材CD	芯材カット..."></textarea>
            
            <div style="text-align:right; margin-top:0.5rem;">
               <button class="btn btn-sm btn-outline" onclick="downloadBomCsvTemplate()">📥 CSVテンプレートをダウンロード</button>
            </div>
        </div>
    `;
  const footer = `
...
function renderReport() {
  ...
  if (filterEnd) {
    ...
    filteredOrders = filteredOrders.filter(o => {
      const d = parseDate(o.dueDate);
      return d && d <= filterEnd;
    });
  }

  // 不良数集計 (期間内)
  let filteredDefects = defects.filter(d => {
    const dDateStr = d.createdAt || d.date || '';
    const dDate = parseDate(dDateStr);
    if (!dDate) return false;
    
    let matchStart = true;
    let matchEnd = true;
    if (filterStart) matchStart = dDate >= filterStart;
    if (filterEnd) matchEnd = dDate <= filterEnd;
      ...
      return `
        <div style="margin-bottom: 0.75rem;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
            ...
            <span style="font-size: 0.875rem; font-weight: 500;">¥${Math.round(data.total).toLocaleString()}</span>
          </div>
      ...
      <div class="report-section" style="background: var(--color-bg-secondary); padding: 1.5rem; border-radius: var(--radius-lg); margin-bottom: 2rem;">
        ...
        <div class="report-summary" style="margin-bottom: 1.5rem;">
          <div class="summary-item">
            ...
            <span class="summary-value" style="color: var(--color-primary);">¥${Math.round(totalInvAmount).toLocaleString()}</span>
          </div>
          <div class="summary-item">
            <span class="summary-label">通常在庫</span>
            <span class="summary-value" style="color: var(--color-success);">¥${Math.round(totalNormalAmount).toLocaleString()}</span>
          </div>
          <div class="summary-item">
            <span class="summary-label">不動品在庫</span>
            <span class="summary-value" style="color: var(--color-warning);">¥${Math.round(totalFixedAmount).toLocaleString()}</span>
          </div>
        </div>
...
function submitInventoryCount() {
  ...
  if (!product) {
    showQuickProductRegisterModal(productId, quantity, true);
    return;
  }
...
window.submitQuickProductRegister = function(productId, isMobile) {
  ...
  if (isMobile) {
    const scanIdInput = $('#inv-scan-id') || $('#inv-scan-product-id');
    if (scanIdInput) scanIdInput.value = '';
    const scanQtyInput = $('#inv-scan-qty') || $('#inv-scan-quantity');
